...
export default function TherapistProfile() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", bio: "", specialization: "", hourlyRate: "" });
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          ...
          <Input id="specialization" label="Specialization" value={form.specialization} onChange={(e) => setForm((p) => ({ ...p, specialization: e.target.value }))} />
          <Input id="hourlyRate" label="Hourly Rate ($)" type="number" value={form.hourlyRate} onChange={(e) => setForm((p) => ({ ...p, hourlyRate: e.target.value }))} />
          <div>
          ...
          <Button onClick={save} loading={saving}><Save className="w-4 h-4 mr-2" /> Save Changes</Button>
        </div>
      </Card>
    </div>
  );
